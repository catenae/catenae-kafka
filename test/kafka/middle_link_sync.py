...
class MiddleLinkSync(Link):
    ...
    def instance_callback_args(self, arg1, arg2):
        logging.debug(
            f'{self.__class__.__name__} -> Callback called, args: {arg1}, {arg2}'
        )

    def setup(self):
        logging.debug(f'{self.__class__.__name__} -> setup()')
        logging.debug(
            f'{self.__class__.__name__} -> input_topics: {self.input_topics}')
        logging.debug(
    ...
    def transform(self, electron):
        ...
        logging.debug(
            f'{self.__class__.__name__} -> previous topic: {electron.previous_topic}'
        )
        ...
        if option == 0:
            ...
            return electron, self.instance_callback_args, [
                'instance_arg_val1', 'instance_arg_val2'
            ]
        elif option == 6:
            return electron, self.instance_callback_args, {
...
if __name__ == "__main__":
    MiddleLinkSync(log_level='DEBUG').start(
        consumer_timeout=20000, synchronous=True)
