...
class Link:
    ...
    def __init__(self,
        ...
        self._rpc_topics = [
            self._rpc_instance_topic, self._rpc_group_topic,
            self._rpc_broadcast_topic
        ]
        self._known_message_ids = CircularOrderedSet(50)

        self._load_args()
        self._set_execution_opts(input_mode, exp_window_size, synchronous,
                                 sequential, num_rpc_threads, num_main_threads,
                                 input_topics, output_topics, kafka_endpoint,
                                 consumer_timeout)
    ...
    @suicide_on_error
    def _set_execution_opts(self, input_mode, exp_window_size, synchronous,
                            sequential, num_rpc_threads, num_main_threads,
                            input_topics, output_topics, kafka_endpoint,
                            consumer_timeout):
    ...
    def _check_instances(self):
        ...
        for uid, properties in known_instances.items():
            self.logger.log(f"checking instance availability for {uid}",
                            level='debug')
    ...
    def rpc_call(self, uid, method, kwargs=None, request_id=None):
        ...
        result = None
        try:
            response_data = urlopen(Request(url=url,
    ...
    def _rpc_notify(self, electron, commit_callback):
        ...
        if not 'method' in electron.value:
            self.logger.log(f'invalid RPC invocation: {electron.value}',
                            level='error')
            return
    ...
    def suicide(self, message=None, exception=False):
        self.launch_thread(self._suicide,
                           kwargs={
                               'message': message,
                               'exception': exception
                           })
        ...
        while not current_thread().will_stop:
            try:
                electron = self._output_messages.get(
                    timeout=Link.QUEUE_GET_TIMEOUT, block=False)
    ...
    def _produce(self, electron, synchronous=None):
        ...
        try:
            ...
            producer.produce(topic=electron.topic,
                             key=partition_key,
    ...
    def _transform(self, electron, commit_callback):
        try:
            ...
            self.suicide('exception during the execution of transform()',
                         exception=True)
    ...
    def _input_handler(self):
        while not current_thread().will_stop:
            self.logger.log('waiting for a new electron to transform...',
                            level='debug')
            ...
            if electron.previous_topic in self._rpc_topics:
                ...
                if electron.value['context']['uid'] == self._uid:
                    ...
                    self._transform_rpc_executor.submit(
                        self._rpc_notify, [electron, commit_callback])
            else:
                self._transform_main_executor.submit(
                    self._transform, [electron, commit_callback])
    ...
    def _is_message_known(self, message):
        ...
        if message_id in self._known_message_ids:
            self.logger.log(
                f'Received known message (topic_partition_offset): {message_id}',
                level='debug')
            return True
        return False
    ...
    def _commit_kafka_message(self, consumer, message):
        ...
        attempts = 1
        self.logger.log(
            f'trying to commit a message ({attempts}/{Link.MAX_COMMIT_ATTEMPTS})',
            level='debug')

        while not commited:
            if attempts > Link.MAX_COMMIT_ATTEMPTS:
                self.suicide(
                    'the maximum number of attempts to commit the message has been reached',
                    exception=True)
    ...
    def _kafka_rpc_consumer(self):
        ...
        self.logger.log(
            f'[RPC] consumer properties: {utils.dump_dict_pretty(properties)}',
            level='debug')
    ...
    def _kafka_main_consumer(self):
        ...
        consumer = Consumer(properties)
        self.logger.log(
            f'[MAIN] consumer properties: {utils.dump_dict_pretty(properties)}',
            level='debug')
        ...
        while not current_thread().will_stop:
            ...
            current_input_topic_assignments = dict(
    ...
    def send(self,
        ...
        if isinstance(output_content, Electron):
            ...
            electron = Electron(value=output_content,
                                topic=topic,
    ...
    def start(self, embedded=False, startup_text=None, setup_kwargs=None):
        ...
        try:
            ...
            try:
                self.suicide('exception during the execution of setup()',
                             exception=True)
            except SystemExit:
                pass
        finally:
    ...
    def _setup_kafka_producers(self):
        sync_producer_properties = dict(
            self._kafka_producer_synchronous_properties)
    ...
    def _launch_tasks(self):
        ...
        if self._kafka_endpoint:
            ...
            self._consumer_rpc_thread = Thread(self._thread_target,
                                               kwargs=consumer_kwargs)
            self._consumer_rpc_thread.start()

            # Kafka main consumer
            self._set_input_topic_assignments()
            self._consumer_main_thread = Thread(
                self._thread_target,
            ...
            self._producer_thread = Thread(self._thread_target,
                                           kwargs=producer_kwargs)
            self._producer_thread.start()

            # Transform
            self._transform_rpc_executor = ThreadPool(self,
                                                      self._num_rpc_threads)
            self._transform_main_executor = ThreadPool(self,
                                                       self._num_main_threads)
            transform_kwargs = {'target': self._input_handler}
            self._input_handler_thread = Thread(self._thread_target,
    ...
    def _report_existence(self):
        ...
        self.rpc_notify(to='broadcast',
                        method='report_existence',
                        kwargs=kwargs)
    ...
    def _set_connectors(self):
        ...
        try:
            self._mongodb = MongodbConnector(self._mongodb_host,
                                             self._mongodb_port,
                                             connect=True)
        except AttributeError:
            self._mongodb = None
    ...
    def _set_jsonrpc_props(self):
        self._jsonrpc_props = {
            'host':
            environ['JSONRPC_HOST'] if 'JSONRPC_HOST' in environ else '0.0.0.0',
            'port':
            environ['JSONRPC_PORT'] if 'JSONRPC_PORT' in environ else 9494,
            'scheme':
            environ['JSONRPC_SCHEME'] if 'JSONRPC_SCHEME' in environ else 'http'
        }
    ...
    def _set_kafka_common_properties(self):
        ...
        self._kafka_consumer_common_properties.update({
            ...
            'session.timeout.ms':
            10000,  # heartbeat thread
            'max.poll.interval.ms':
            self._consumer_timeout,  # processing time
            'enable.auto.commit':
            True,
        ...
        self._kafka_consumer_synchronous_properties = dict(
            self._kafka_consumer_common_properties)
        ...
        self._kafka_producer_synchronous_properties = dict(
            self._kafka_producer_common_properties)
